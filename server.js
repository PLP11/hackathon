...
//hellooooo
app.post('/store_course', (req, res) => {
    const { course } = req.body;
    const sql = 'INSERT INTO courses (course_name) VALUES (?)';
    connection.query(sql, [course], (err, result) => {
        if (err) {
            console.error('Error storing course in database:', err);
            res.status(500).send('Error storing course in database');
            return;
        }
        console.log('Course stored in database:', course);
        res.sendStatus(200);
    });
});
